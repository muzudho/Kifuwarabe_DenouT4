...
namespace Grayscale.A630_GuiCsharp__.B110_ShogiGui___.C250____Timed
{
    ...
    public class TimedB_MouseCapture : Timed_Abstract
    {
        ...
        public static void Check_MouseoverKomaKiki(object obj_shogiGui, Finger finger, KwLogger errH)
        {
                                        ...
                                        foreach (Shape_BtnKomaImpl btnKoma in mainGui.Shape_PnlTaikyoku.Btn40Komas)
                                        {
                                            if (btnKoma.HitByMouse(eventState.MouseLocation.X, eventState.MouseLocation.Y))
                                            {
                                                ...
                                                if (Okiba.ShogiBan == Conv_Busstop.ToOkiba(koma1))
                                                {
                                                    ...
                                                    //マウスの左ボタンを放したときです。
                                                    if (!mainGui.Link_Server.KifuTree.CurChildren.ContainsKey(newNode.Key))
                                                    {
                                                        //----------------------------------------
                                                        // 次ノード追加
                                                        //----------------------------------------
                                                        mainGui.Link_Server.Earth.GetSennititeCounter().CountUp_New(Conv_Sky.ToKyokumenHash(sky_newChild), "TimedB.Step(1)");
                                                        mainGui.Link_Server.KifuTree.AddCurChild(newNode.Key, newNode, mainGui.Link_Server.KifuTree.CurNode3okok);
                                                    }

                                                    string jsaFugoStr;
                                                    mainGui.Link_Server.KifuTree.OnDoMove(newNode, sky_newChild);
                                                    ...
                                                    {
                                                        mainGui.ChangedTurn(
                                                            mainGui.Link_Server.KifuTree.CurNode3okok,//mainGui.Link_Server.KifuTree.CurNode2ok,
